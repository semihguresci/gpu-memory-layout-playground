...
def _build_output_path(payload: dict, runs_dir: Path, label: str) -> Path:
    ...
    suffix = 2
    while True:
        alternate_name = f"{timestamp_token}_{suffix}.json" if not label_slug else f"{timestamp_token}_{label_slug}_{suffix}.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collect Experiment 01 JSON output into runs/<device>/...")
    parser.add_argument(
        "--input",
        type=Path,
        default=DEFAULT_INPUT,
        help="Input benchmark JSON path (default: experiments/01_dispatch_basics/results/tables/benchmark_results.json).",
    )
    parser.add_argument(
        "--runs-dir",
        type=Path,
        default=DEFAULT_RUNS_DIR,
        help="Destination runs directory (default: experiments/01_dispatch_basics/runs).",
    )
    parser.add_argument(
        "--label",
        type=str,
    ...
    if not any(str(row.get("experiment_id", "")) == EXPERIMENT_ID for row in rows):
        raise ValueError(
            "Input JSON has rows but no Experiment 01 entries. Run with --experiment 01_dispatch_basics first."
        )
